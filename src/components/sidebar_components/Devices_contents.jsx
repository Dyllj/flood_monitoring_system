...
const Devices_contents = ({ isAdmin }) => {
  ...
  // ✅ Real-time listener for Firestore (auto updates when device is added/removed/edited)
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "devices"), (snapshot) => {
      const updatedDevices = [];
      snapshot.forEach((doc) => updatedDevices.push({ id: doc.id, ...doc.data() }));
  ...
  // ✅ Real-time listener for each device in RealtimeDB
  useEffect(() => {
    const listeners = [];
  ...
  // ✅ Edit Device Metadata (Firestore only)
